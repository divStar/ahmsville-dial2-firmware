 ...
 *
 * This class is a ISchedulableDialTask, that handles the cleanup of the
 * messages the InputProcessorTask produces. <p>In particular it deletes all
 * messages, that are invalid. See InputMessageDto for more details on when a
 * message is valid.</p>
 *
 * @author  Igor Voronin
 * @date    28.07.2023
...
public:
   ...
   * list containing the messages to be processed
   */
...
private:
  ...
  bool isValidData(JsonVariantConst jsonData) override { /* there is no data to validate */
    return false;
  };
};
