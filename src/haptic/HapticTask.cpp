...
HapticTask::HapticTask(IHapticSensorAdapter &sensorAdapter, LinkedList<InputMessageDto *> &messagesToProcess)
    : ISchedulableDialTask("haptic"), ISerialPortUser(configuredSerialPort()), IMessageConsumer(messagesToProcess),
...
bool HapticTask::isValidData(JsonVariantConst jsonData) { return jsonData["type"] == getTaskType() && jsonData["strength"].is<byte>(); }
