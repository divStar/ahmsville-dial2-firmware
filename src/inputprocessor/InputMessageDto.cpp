...
char *InputMessageDto::getRawInputData() {
    return rawInputData;
}

bool InputMessageDto::isValid() const {
    return itemValid && (millis() - millisSinceBirth < LIFE_SPAN);
}

void InputMessageDto::setValid(bool validity) {
    itemValid = validity;
}