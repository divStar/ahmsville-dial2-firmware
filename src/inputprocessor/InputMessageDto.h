 ...
 * @brief   DTO (data transfer object) used to transport a message from the
 * Serial port to a sensorAdapter task.
 *
 * This class describes a DTO (data transfer object), which is used to transport
 * a message from the Serial port to a sensorAdapter task. It also provides a
 * function to make it possible to determine whether this object is still valid
 * or subject to be pruned by the MessagesCleanerTask.
 *
 * @author  Igor Voronin
 * @date    28.07.2023
 */
...
public:
   ...
   *
   * @return (bool) <code>true</code> if this message is still valid,
...
private:
  ...
  /**
   * @brief Flag, that shows whether this message is still to be considered
   * valid or not.
   */
  bool itemValid = true;
};
