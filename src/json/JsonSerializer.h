 ...
 *
 * This class handles the json serialization to JSON. It also sends the serialized JSON to a Serial port.
 *
 * @author  Igor Voronin
 * @date    30.07.2023
 */
...
public:
    /**
     * @brief Serializes the given type, info and message references to a JSON message and sends it to the Serial port.
     *
     ...
     * @param info      (const char*) info(number) (`info` attribute in the resulting JSON)
     ...
     * @param serial    (Stream) reference to the Serial port to output to
     * @param type      (const char*) message type (`type` attribute in the resulting JSON)
     * @param error     (const char*) error(number) (`error` attribute in the resulting JSON)
     * @param message   (const char*) message (`message` attribute in the resulting JSON)
     */
