...
LedTask::LedTask(LinkedList<InputMessageDto *> &messagesToProcess)
    : ISchedulableDialTask("led"), ISerialPortUser(configuredSerialPort()), IMessageConsumer(messagesToProcess) {
  setInterval(0);
...
void LedTask::onCallback() {
  for (int rawDataDtoIndex = messagesToProcess.size() - 1; rawDataDtoIndex >= 0; --rawDataDtoIndex) {
    ...
    if (strstr(rawDataDto->getRawInputData(), taskType) != nullptr) {
      ...
      DeserializationError error = deserializeJson(jsonDoc, rawDataDto->getRawInputData(), strlen(rawDataDto->getRawInputData()),
                                                   DeserializationOption::Filter(filterDoc));
      if (error) {
        char errorMessage[150];
        sprintf(errorMessage, "[LED] deserializeJson() failed: %s", error.c_str());
        JsonSerializer::sendJsonError(serial, getTaskType(), "L1", errorMessage);
...
void LedTask::useData(JsonVariantConst jsonData) {
  ...
  for (JsonObjectConst ledDataJson : ledDataJsonArray) {
    if (isValidData(ledDataJson)) {
      Log.traceln("[LED] Setting leds with (index, r, g, b, brightness) to "
                  "(%d, %d, %d, %d, %d)",
                  ledDataJson["ledIndex"].as<byte>(), ledDataJson["toColor"][0].as<byte>(), ledDataJson["toColor"][1].as<byte>(),
                  ledDataJson["toColor"][2].as<byte>(), ledDataJson["misc"]["brightness"].as<byte>());

      leds[ledDataJson["ledIndex"].as<byte>()].setRGB(ledDataJson["toColor"][0].as<byte>(), ledDataJson["toColor"][1].as<byte>(),
                                                      ledDataJson["toColor"][2].as<byte>());

      leds[ledDataJson["ledIndex"].as<byte>()].maximizeBrightness(ledDataJson["misc"]["brightness"].as<byte>());
...
bool LedTask::isValidData(JsonVariantConst jsonData) {
  bool isLedIndexValid = jsonData["type"] == getTaskType() && jsonData["ledIndex"].is<LedIndexEnum>() && jsonData["ledIndex"] >= 0 &&
                         jsonData["ledIndex"] <= NUM_LEDS - 1;
  if (!isLedIndexValid) {
...
void LedTask::sendValidationError(const char *error, const char *message, byte data) {
  ...
  JsonSerializer::sendJsonError(serial, getTaskType(), error, validationErrorMessage);
